	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == Jbtn_enter) {
			...
			if(subjectName.equals("") || selectedSort.equals(null)) {
				...
				Jlb_result.setText("沒有" + subjectName + "之成績");
				Jlb_result.setForeground(Color.RED);
				jSubjectName.setText("");
			}
			else {
				//根據輸入的科目名稱及排序方法進行成績排序
				int[] sortInfo = {0, 0};//紀錄排序的交換和比較次數
				int[][] data = GradeSystem.getStudentGrade(subjectName);
				String[][] strData;
				...
				switch(selectedSort) {
					case "Insertion Sort":
						...
						Jlb_result.setForeground(Color.BLACK);
